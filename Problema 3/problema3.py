...
Problema 3(a): Newton–Raphson en 2D para la función de Rosenbrock desplazada

    f(x, y) = (x - 1)**2 + 100*(y - x**2)**2

Requisitos:
- Calcular analíticamente ∇f y ∇²f.
- Implementar Newton–Raphson 2D desde cero (sin funciones de optimización).
- Punto inicial (x0, y0) = (0, 10).
- Graficar z=f(x,y) y superponer la ruta de iteraciones, marcando el mínimo final en rojo.

Librerías permitidas: NumPy, SymPy, Matplotlib.
Salidas:
    - problema3a_superficie_ruta.png
    - problema3a_contornos_ruta.png
    - problema3a_iteraciones.csv
"""
...
# ===================== 3) Newton–Raphson en R^2 (desde cero) ==================
def newton2d(f, grad, hess, x0, y0, alpha=0.6, tol=1e-10, max_iter=300):
    ...
    Parámetros:
        f, grad, hess : callables -> f(x,y), ∇f(x,y), ∇²f(x,y)
        x0, y0 : float -> punto inicial
        alpha ∈ (0,1] : factor de amortiguación (mejora estabilidad global)
        tol : float -> tolerancia sobre ||∇f||_2
        max_iter : int -> máximo de iteraciones
    Retorna:
        dict con listas de x, y, f, ||∇f|| y número de iteraciones.
    """
    ...
    for k in range(max_iter):
        ...
        # Norma del gradiente (criterio de paro)
        gn = float(np.linalg.norm(gk, 2))
        gns.append(gn)
        if gn < tol:
            break

        # Dirección de Newton: resolver Hk * p = gk (evitar invertir explícitamente Hk)
        try:
            pk = np.linalg.solve(Hk, gk)
        except np.linalg.LinAlgError:
            # Hessiana singular o muy mal condicionada
            print(f"Advertencia: Hessiana singular/mal condicionada en la iteración {k}.")
            break
...
# ===================== 4) Ejecutar Newton desde (0,10) ========================
hist = newton2d(f, grad, hess, x0=0.0, y0=10.0, alpha=0.6, tol=1e-10, max_iter=300)

# Guardar log de iteraciones a CSV (k, x_k, y_k, f, ||grad||)
...
# ===================== 5) Gráfica 3D de la superficie con ruta =================
def plot_superficie_con_ruta(hist, xlim=(-2, 2), ylim=(-1, 11),
    ...
    # Ruta de Newton en 3D (x,y,f(x,y))
    xs, ys = np.array(hist['x']), np.array(hist['y'])
    zs = f(xs, ys)
    ax.plot(xs, ys, zs, linewidth=2)
    ax.scatter(xs[-1], ys[-1], zs[-1], s=60)  # punto final (mínimo) en rojo
...
# ===================== 6) Gráfica de contornos con ruta =======================
